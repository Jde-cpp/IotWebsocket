create or alter proc opc_server_insert @name varchar(255), @target varchar(255), @attributes smallint, @description varchar(2047), @is_default bit, @certificate_uri varchar(2047), @url varchar(2047) as begin
	set nocount on;
  if @is_default=1
...
go
create or alter trigger opc_server_update on opc_servers for update as begin
    declare @is_default bit, @id int;
    select @is_default=is_default, @id=server_id from inserted;
    if @is_default=1
        update opc_servers set is_default=0 where server_id!=@id;
end
go